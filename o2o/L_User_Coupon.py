...
def get_L_User_Coupon_Feat(dataframe_L,dataframe):
    ...
    # 上旬领券次数
    pivot = pd.pivot_table(data[data.date_received.map(lambda x : x.day >= 1 and x.day < 11)],index = keys,values = 'Discount_rate',aggfunc = len)
    ...
    # 中旬领券次数
    pivot = pd.pivot_table(data[data.date_received.map(lambda x : x.day >= 11 and x.day < 21)],index = keys,values = 'Discount_rate',aggfunc = len)
    ...
    # 下旬领券次数
    pivot = pd.pivot_table(data[data.date_received.map(lambda x : x.day >= 21)],index = keys,values = 'Discount_rate',aggfunc = len)
