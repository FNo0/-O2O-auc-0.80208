...
def RC_User_Discount_Feat(dataframe_RC,dataframe):
    ...
    pivot.rename(columns = {'distance' : prefixs + 'ReceiveAndConsume_Coupon_mean_distance'},inplace = True)
    dataset = pd.merge(dataset,pivot,left_on = keys,right_index = True,how = 'left')
    dataset[prefixs + 'ReceiveAndConsume_Coupon_mean_distance'].fillna(-1,downcast = 'infer',inplace = True) #填充缺失值
    # 在多少不同商家领取优惠券并消费
    pivot = pd.pivot_table(data[data.Date != 'null'],index = keys,values = 'Merchant_id',aggfunc = lambda x : len(set(x)))
    pivot.rename(columns = {'Merchant_id' : prefixs + 'ReceiveAndConsume_in_different_Merchant_cnt'},inplace = True)
    dataset = pd.merge(dataset,pivot,left_on = keys,right_index = True,how = 'left')
    dataset[prefixs + 'ReceiveAndConsume_in_different_Merchant_cnt'].fillna(0,downcast = 'infer',inplace = True) #填缺失值0表示没领
    # 在多少不同商家领取优惠券
    pivot = pd.pivot_table(data,index = keys,values = 'Merchant_id',aggfunc = lambda x : len(set(x)))
    pivot.rename(columns = {'Merchant_id' : prefixs + 'Receive_in_different_Merchant_cnt'},inplace = True)
    dataset = pd.merge(dataset,pivot,left_on = keys,right_index = True,how = 'left')
    dataset[prefixs + 'Receive_in_different_Merchant_cnt'].fillna(0,downcast = 'infer',inplace = True) #填缺失值0表示没领
    # 在多少不同商家领取优惠券并消费 / 在多少不同商家领取优惠券
    dataset[prefixs + 'ReceiveAndConsume_in_different_Merchant_cnt_div_Receive_in_different_Merchant_cnt'] = dataset[prefixs + 'ReceiveAndConsume_in_different_Merchant_cnt'] / (dataset[prefixs + 'Receive_in_different_Merchant_cnt'] + 0.1)
    # 领取多少不同种优惠券
